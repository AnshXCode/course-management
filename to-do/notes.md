...
"HTTP caching reduces requests that reach the server — great for public static assets and CDN. Redis is application-level caching: shared across API instances, works with auth, fine-grained invalidation, and protects the database. They’re complementary; for a JWT-protected API, Redis does what HTTP caching can’t safely do at the browser or edge."